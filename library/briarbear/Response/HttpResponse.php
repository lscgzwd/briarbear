...
class HttpResponse extends Response
{
    ...
    public function sendFile($file, $attachmentName = '', $attach = true)
    {
        ...
        if ($attach === true) {
            ...
            $this->setHeader('Cache-Control', 'max-age=' . \BriarBear::$server->httpStaticCacheTime)
                ->setHeader('Pragma', 'cache')
                ->setHeader('Last-Modified', date('D, d M Y H:i:s \G\M\T', filemtime($file)))
                ->setHeader('Expires', date('D, d M Y H:i:s \G\M\T', time()+\BriarBear::$server->httpStaticCacheTime));
        }
    ...
    public function getHeader($fastcgi = false)
    {
        ...
        if (!isset($this->head['Server'])) {
            $this->head['Server'] = \BriarBear::$server->serverName . ' ' . \BriarBear::getVersion();
        }
