...
class Crontab extends Object
{
    ...
    public function run($server)
    {
        ...
        \BriarBear::info('run crontab');
        $this->server = $server;

        $this->zk = \BriarBear::createObject([
    ...
    public function loadCron()
    {
        try {
            if ($this->zk->isMaster === true && $this->server->swooleServer !== null) {
                \BriarBear::info('master_load_cron');
                $time = time();
                foreach ($this->cronList as $key => $cron) {
                    ...
                    if ($execTime) {
                        \BriarBear::info('crontab_next_minute_run:' . json_encode($cron));
                        CronHeap::setTask($execTime, $cron, $this->zookeeperNamespace);
                    }
                }
            }
        } catch (\Throwable $e) {
            \BriarBear::error('loadCronFail:' . $e->__toString());
        }
        return true;
    }
    /**
    ...
    public function doCron()
    {
        try {
            if ($this->zk->isMaster === true && $this->server->swooleServer !== null) {
                ...
                if (!empty($tasks)) {
                    \BriarBear::info('crontab_do_this_second:' . json_encode($tasks));
            ...
            \BriarBear::error('doCronFail:' . $e->__toString());
        }
        return true;
    }
