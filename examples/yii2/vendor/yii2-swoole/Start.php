...
defined('BRIARBEAR_PATH') || define('BRIARBEAR_PATH', ROOT_PATH . '/library/briarbear/');
// 配置分离，从OP管理的配置中获取配置
...
class Start
{
    ...
    public function run()
    {
        require rtrim(BRIARBEAR_PATH, '/') . '/BriarBear.php';
        $this->initConfig();
        $this->loadYii();
    ...
    {
        // 加载Yii核心
        require rtrim(VENDOR_PATH, '/') . '/autoload.php'; // 自动加载
        require rtrim(VENDOR_PATH, '/') . '/yiisoft/yii2/Yii.php'; // Yii核心类
        require $this->yiiBootstrapFile; // 命名空间注册
        \Yii::setAlias('@yiiswoole', VENDOR_PATH . '/yii2-swoole');
        \Yii::setAlias('@yiilog', VENDOR_PATH . '/yii2-log');
