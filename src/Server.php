...
class Server extends Object
{
            ...
            switch ($action) {
                case 'start':
                    ...
                    if (!empty($masterPid) && posix_kill($masterPid, 0)) {
                        \BriarBear\BriarBear::info('Server already running, exit.');
                        return;
                    }
                    $this->startListen();
                    break;
                case 'restart':
                    ...
                    if (empty($masterPid) || !posix_kill($masterPid, 0)) {
                        \BriarBear\BriarBear::info('Server not running, start it.');
                    } else {
                        $tick = 0;
                        while (true) {
                            if ($tick < 10) {
                ...
                case 'stop':
                    ...
                    $managerPid = $pidArr['managerPid'];
                    if (empty($masterPid) || !posix_kill($masterPid, 0)) {
                        \BriarBear\BriarBear::info('Server not running, exit.');
                    } else {
                        $tick = 0;
                        while (true) {
                            if ($tick < 10) {
                    ...
                    break;
                default:
                    \BriarBear\BriarBear::error("Unknown command;Usage: Path_to_php/php {$argv[0]} start|reload|stop, exit.");
                    return;
                    break;
            }
        } catch (\Throwable $e) {
            echo var_export($e->__toString(), true) . PHP_EOL;
        }
    }
    protected function initConfig()
    {
        ...
        if (is_array($this->webSocket) &&
            ...
            \BriarBear\BriarBear::info('Bad config, web socket host, port or message callback not set. skip');
            $this->webSocket = false;
        } else {
            $webSocket = true;
        }
        if ($webSocket === false && (empty($this->host) || empty($this->port) || !is_numeric($this->port))) {
            \BriarBear\BriarBear::info('Bad config, no server config given, nothing need to start, exit.');
            return false;
        }
    ...
    protected function startListen()
    {
        ...
        if (null === $server) {
            \BriarBear\BriarBear::info('No server config given, exit.');
            return;
        }
    ...
    public function onShutdown(\Swoole\Server $server)
    {
        ...
        if (empty($masterPid) || !posix_kill($masterPid, 0)) {
            \BriarBear\BriarBear::info('Server stopped success, exit.');
            unlink($this->pidFile);
        } else {
            \BriarBear\BriarBear::info("Server stopped failed, exit. PID: {$masterPid}");
        }
    }
    /**
        ...
        try {
            ...
            switch ($_SERVER['REQUEST_METHOD']) {
                ...
                case 'POST':
                    ...
                    $this->checkContentLength($buffer->getAcceptBodyLength());
                    if (isset($_SERVER['CONTENT_LENGTH']) && $_SERVER['CONTENT_LENGTH'] == $buffer->getAcceptBodyLength()) {
                        if (isset($_SERVER['CONTENT_TYPE']) && ($_SERVER['CONTENT_TYPE'] == 'application/x-www-form-urlencoded' || strpos($_SERVER['CONTENT_TYPE'], 'application/x-www-form-urlencoded') === 0)) {
                            ...
                            if ($this->logRawPost) {
                                ...
                                $buffer->getRequest()->setRawType(Request::RAW);
                            }
                        }
            ...
            \BriarBear\BriarBear::error('Connection was closed and exception caused:' . $e->__toString());
            if (!isset($_SERVER['REQUEST_METHOD'])) {
            ...
            if ($this->gzip && $response->getBodyLength() > $this->gzipMinLength) {
                if (!empty($headers['Accept-Encoding'])) {
                    ...
                    elseif (strpos($headers['Accept-Encoding'], 'deflate') !== false) {
                        ...
                        \BriarBear\BriarBear::info("Unsupported compression type : {$headers['Accept-Encoding']}.", 'info');
                    }
                }
            }
